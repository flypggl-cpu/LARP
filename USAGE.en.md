...
- When going to **second-pass detailed analysis**, also paste [`prompts/LARP_modules.en.md`](prompts/LARP_modules.en.md) (the criteria & check modules). The body alone is enough through the first pass (the argument map).
- For **structure only**, use LARP-Map ([`prompts/LARP_map.en.md`](prompts/LARP_map.en.md)); for a **quick check**, the lite edition — Map = structure / Lite = check / full body = deep analysis.
