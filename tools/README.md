...
> **실측 한 조각.** 한 실제 항소심 판결문(정답키 182개 항목)에 돌려 재현율을 채점했더니 — 소넷은 전체 **약 96%**(쟁점 100%·증거 96%), 하이쿠 첫 판은 **약 56%**였습니다(모델 차이가 큽니다). 그런데 그 96%짜리 출력도 **실제 항목 7개를 조용히 빠뜨렸고**, 그건 *읽어서는* 안 보이고 정답키 대조로만 드러났습니다 — 이 폴더의 코드 검사가 필요한 이유가 바로 이것입니다. (재현율은 '진실'이 아니라 '정답키 대비 빠짐없음'을 재는 값입니다.)
