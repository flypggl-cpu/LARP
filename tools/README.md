...
이 폴더는 전체판 LARP의 **검증 층(LARP.md §3.7)의 코드 부분**입니다. 1차 출력 뒤, 사람이 보고도 못 거르는 위험을 **매번 똑같이·기계적으로 짚어내 *눈에 보이게*** 만드는(즉 결정론적으로) 보조 도구가 들어 있습니다. 각 도구가 정확히 *무엇을 하는지*를 아래 표로 정리하고, 실행법(모두 공통)과 상세 사용서는 그 아래에 있습니다.
