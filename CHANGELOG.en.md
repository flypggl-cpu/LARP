...
- **[measured] Gate 0 now prints the full evidence-seed list to the console** (`tools/larp_gate0.py`; suppress with `--brief`). It used to print only the count, with the list available in `--md`/`--json` alone. Contract ② (seeds n = tree E n) demanded a reconciliation **while the list never reached the executor**. Discovered when the first run reported contract ② as "cannot perform". Grounds: seed recall 55% → 96% (list + machine comparison).
- **[measured] §3.7 gains an in-window machine-comparison branch.** Of the three things bundled as "independent verification", **a·b (quote-source and coverage comparison) are deterministic string comparison and never needed independence** — whether a string is in the source reads the same to any eye. Independence is required for *judgment*, not for *comparison*. Where search tools exist, string-search the seeds immediately after the draft, **in the same turn**, and print three numbers: `seeds n / in draft m / added after machine comparison k`. Only c (the semantic omission hunt) genuinely needs a fresh window or different model. Gate 4 corrected likewise. Grounds: one run caught 19 of its own omissions with Grep, moving 74% → 96%. Decomposed: handing over the list +9pp, mandating the comparison +32pp — **the list alone gets you less than halfway.**
- **[measured] Omission re-sweep — 'writing it in the log is not registering it'** (§3.10 + hard limit ⑧). In the old wording, *"retroactively register into the tree … list only the added items"*, the second half read as the deliverable, and the contract numbers `[found k / in tree m / added n]` were **all three self-contained inside the re-sweep log**. (c) now reads "actually write it into the tree body as a new E row", and the sweep must end with the reconciliation **`total added n = E rows newly standing in the tree body n`**. Grounds: even given the seed list, a run reached 84% by mention but 64% by actual inclusion — a 20-point leak, 12 of whose 14 items sat inside the re-sweep log. After the fix the gap is 0pp.
- **[measured] Contract ①'s baseline moves from 'table of contents' to 'the document's own issue roster'** (§3.10 + hard limit ① + new gate-0 step ②-1). Priority: the section where the document enumerates its issues (appellate = "summary of grounds of appeal" / first instance = "summary of the charges" / paper = research questions in the introduction / report = summary or recommendations) → table of contents → otherwise **state 'no external baseline — issue count unverified'**. Gate 0 locates that section and hands it over so the model does not invent a baseline. Grounds: when the model sets the baseline the contract becomes an identity — across six runs A rows ranged 44–183 (4.2×) and **not one tripped contract ①**, yet reconciling afterwards against the "summary of grounds of appeal" showed all six carried 23–24 of 24 issues. **The content was fine; only the contract was inert.** This also closes G-4 (contract tied to the judgment profile) — the tool now has a baseline for papers and reports.
- **[measured] Party filings excluded from gate-0 evidence seeds** (opinions, appellate briefs, supplements, answers, written arguments). A brief belongs to the lineage of claims, not the evidence roster — its absence from the tree E rows is correct. Seeds 73 → 69.
...
- **[measured] Stage 0 (skeleton · pre-registration) added; Stages 0 and 1 split into separate turns.** §7.9 required the top-down skeleton *"before close reading"*, but §3.10 placed it at Stage 2 — **after** Stage 1's exhaustive evidence sweep. That is exactly what §3.5-6 forbids: *"the arrangement you already saw conditions the prediction."* Stage 0 now sits right after Gate 0, with the run-card, turn plan, §3.5-6 and §7.9 all reconciled.
- **[measured] The turn-split grounds are stated in the body — the first turn's budget is fixed and the contracts decide its allocation.** With Stages 0 and 1 bundled, one run kept the four lines per A and crushed the re-sweep into 6 large windows (a format violation); another kept the re-sweep and dropped the four lines (0% complete). **Total output was about the same — allocation, not laziness.** Split into two turns, four lines at 100% and a 30-window re-sweep stood together.
- **[measured] Stage 0 [input practice] — run it on an excerpt.** Unlike a human's pre-registration, the model has already seen the text. Two runs given the full text both marked their own pre-registration **'void'** in the order audit. Given only an excerpt of contents, summaries and conclusion (13% of the full text) in a separate turn, 'before close reading' held for the first time (23 issues · 88 pre-registered items · 0 ungraded). Where that is impossible, run it anyway but label the order audit *'run after full-text input — pre-registration force limited'*.
- **[judgment] Order audit (§3.10 interior rule 8) from optional to mandatory.** *"May be shown in the tail"* is not an instruction. On 'no', the run's pre-registration reconciliation is marked void.

### The reconstruction axis — ablated, then kept

- **[measured] Contract ③ strengthened — `claim (A) rows = hidden-assumption (W) rows = other-explanation (H) rows`.** Static analysis called it redundant (three runs without the contract all hit 99–100%) and put it first in line for ablation; running the ablated version, **four-line completeness collapsed to 0%.** The accurate reading is not *"it collapses without this"* but **"it is the safeguard that protects the four lines when the load is high"** — the real fix is lowering the load (turn splitting), and this contract is the insurance until then. Rollback condition: re-ablate once turn splitting has settled; delete if 100% holds.
- **[grounded] The bridge table (§6.5) promoted to a visible artifact of Stage 2.** §7 candidate extraction, §7.3 backward reconstruction and §10 module adjustment **all take this table as input**, yet the run-card left it as "internal work" with no artifact. The useful joint (J node) is its third column — five consumers with zero producers, resolved together. Stage 2 untested, hence [grounded].

### Deletions (0 references confirmed)

- **[judgment] §4 Overall flow deleted** — 0 references in full-text search; only a one-line pointer remained after the body itself noted *"the old summary here was redundant and has been removed."*
- **[judgment] §5.4 Core formation sentence deleted** — 0 references, no separate item in §14.
- **[judgment] Module J (proof-proposition tree) deleted** — no routing entry in §10, 0 references, and functionally identical to the P layer of the §7.9 DB.
- **[judgment] §14 items 1–6 now state where they are produced** (numbering unchanged — §3.9 cites '12-1'). `0-1. Stage 0 skeleton` added.

### What remains (honest labeling)

- **Stages 2–4 were never run.** All six runs stopped at Stage 1. There is no measurement of dialogue (plain-language designation → confirmation question), selection (flags) or judgment (six questions · modules). Most [grounded] items in this release hang on this.
- **Precision was not measured.** Only recall — whether the flags are right, and whether trivial flaws get inflated into risks, is unmeasured.
...
- **Verification-layer default redefined — automatic re-sweep + honest completeness label** (§3.7 + Gate 4, +ko; USAGE §5.3 recast as "pushing completeness higher — the independent layer (optional)"). The same-turn re-sweep the tool *can* run itself (§3.10) runs automatically during the pass; the user is not prompted to opt out. The independent checks (code + separate-model omission hunt) can't self-run in-window, so the Pass-1 output carries an honest label — *'auto re-sweep done · independent verification not run (measured completeness ~96%, some omission possible)'* — and a user wanting higher completeness follows the USAGE independent procedure (fresh-window LARP_verify → feed back, a different model if possible). Output is not labeled 'verified' on the auto re-sweep alone. Grounds: empirical — even a strong model sat at ~96% after the re-sweep, missing name-only testimony/narrative items recovered only by a *different eye*.
