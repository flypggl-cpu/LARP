...
- **Added a usage guide (workflow)** [`docs/workflow.en.md`](docs/workflow.en.md): an at-a-glance "what do you want → which tool, in what order" guide (the 5-step flow for a long argument, tool selection, deep research at both ends, paper vs judgment). Linked from README and USAGE. Roles split: *which-tool-when = workflow / mechanics = USAGE / what & why = README.*
