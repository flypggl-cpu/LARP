...
> - 모델 선택 한 줄: 짧은 글은 아무 모델이나 되지만, **긴 판결문·논문은 Claude Sonnet급 이상**(GPT는 4급 이상, Gemini는 Pro 이상)으로 — 경량판(플래시·라이트·미니류)은 같은 판결문 직접 비교에서 쟁점 17개를 4개로, 증거 108건을 8건으로 뭉개는 것이 실측됐습니다.
