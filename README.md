...
LARP는 어떤 글이든 그 **논증을 재구성해 한 장의 지도로 펼치는** 프롬프트 모음입니다(작은 코드 도구 하나 포함). 글에 *적힌* 주장·논거·증거뿐 아니라, *적히지 않은 채 결론을 떠받치는 것* — 숨은 전제, 다른 설명, 있어야 하는데 빠진 증거 — 까지 끄집어내 보여 줍니다. 판정은 하지 않고 *따져볼 자리*만 짚어, 판단은 사람에게 돌려줍니다. 글의 길이·깊이에 따라 아래 도구들이 그 일을 나눠 맡습니다.
...
| [전체판 LARP](prompts/LARP.md) + [모듈](prompts/LARP_modules.md) | 숨은 전제·*겉이유와 실제이유의 갈림*·다른 설명·빠진 증거를 짚어 약점을 분석 | 고른 논거를 깊이 따질 때 |
| [LARP-Lite](prompts/LARP_lite.md) | 위 분석의 *한 화면 축약판* | 짧은 글을 빠르게 점검 (지름길) |
...
> **다 쓰는 게 아니라, 필요한 것만 고르세요.** 대개는 [LARP-Map](prompts/LARP_map.md)으로 구조를 펼쳐 보는 것으로 충분합니다. 더 깊이 따질 논거가 있으면 [전체판 LARP](prompts/LARP.md)로, "이거냐 저거냐" 경쟁가설을 가려야 하면 [LARP-Weigh](prompts/LARP_weigh.md)로 이어갑니다. 아주 긴 글은 [긴 문서 모드](prompts/LARP_map_long.md)로 도해하고, 증거 빠짐이 걱정되면 [누락 증거 검사](tools/)를 곁들이면 됩니다. 짧은 글은 [LARP-Lite](prompts/LARP_lite.md) 하나로 끝.
>
