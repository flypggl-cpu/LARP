...
> This is an *evaluation* tool — strongest *after* you've laid out the structure with [LARP-Map](LARP_map.en.md) (for long texts, the [full LARP](LARP.en.md)'s evidence→hypothesis DB) and, for long texts, checked for missing evidence with the [`tools/`](../tools/README.en.md) coverage audit. For the whole flow, see [README's 'Which tool, when'](../README.en.md).
