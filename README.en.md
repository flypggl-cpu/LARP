...
> ### ▶ How to use it now
> **Copy the entire contents** of [`prompts/LARP.en.md`](prompts/LARP.en.md) & [`prompts/LARP_modules.en.md`](prompts/LARP_modules.en.md)) and paste them into a chatbot (ChatGPT, Claude, etc.), then paste the text you want analyzed. That's it. (Nothing to install.)
>
...
>
> **Just want the structure at a glance?** Use LARP-Map [`prompts/LARP_map.en.md`](prompts/LARP_map.en.md) — every claim and ground as a tree, no evaluation.
>
> **For deeper work (the full version's second pass)**, also paste the criteria & check modules [`prompts/LARP_modules.en.md`](prompts/LARP_modules.en.md) alongside the body — not a standalone tool but a companion to the full version; the body alone suffices through the first-pass argument map.
...
> Tip: the body is long (~1,250 lines), so for longer documents use a **model with a large context window**. For **second-pass detailed analysis**, also paste [`prompts/LARP_modules.en.md`](prompts/LARP_modules.en.md) (the criteria & check modules) — the body holds only what the first-pass detection/selection needs, and the detailed criteria/routing are split into the modules (the body alone is enough through the first-pass argument map). And if the tool runs straight through instead of stopping after the first pass, just add **"do the first pass only, then stop"** to your first message.
...
| [`prompts/LARP_lite.en.md`](prompts/LARP_lite.en.md) | Lightweight edition — one-screen quick check |
| [`prompts/LARP_map.en.md`](prompts/LARP_map.en.md) | LARP-Map — every claim/ground as a tree, no evaluation (structure only) |
